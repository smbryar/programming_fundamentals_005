...
*/
function sum(object) {
  return Object.values(object).reduce((acc, num) => isNaN(num) ? acc : acc + num, 0);
}
...
function invert(object) {
  ...
  return Object.keys(object).reduce((newObj, key) => {
    //Add new key-value pair to newObj of object[key]:key
...
*/
function addAll(arr) {
  return arr.reduce((finalObj, initialObj) => Object.assign(finalObj, initialObj), {});
}
...
function every(obj, func) {
  return Object.values(obj).every(i => func(i));
}
...
function some(obj, func) { 
  return Object.values(obj).some(i => func(i));
 }
